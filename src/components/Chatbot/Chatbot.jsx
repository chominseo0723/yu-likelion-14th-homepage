...
const STOPWORDS = ['은', '는', '이', '가', '을', '를', '의', '에', '에서', '와', '과', 
                   '도', '로', '으로', '까지', '부터', '한테', '께', '이나', '나', '요',
                   '이요', '네요', '어요', '아요', '해요', '죠', '지요', '나요', '까요',
                   '어', '아', '지', '고', '며', '면', '서', '게', '도록', '한', '그', '저', 
                   '이런', '저런', '어떤', '무슨', '어느', '있', '없', '하']
    ...
    setTimeout(async () => {
      if (answers.length > 0 && answers[0].score >= 15) {
        ...
        if (openai) {
          ...
          setMessages((prev) => prev.filter(msg => msg.text !== '🤔 답변을 생성하고 있습니다...'))
        ...
        <div className="glass w-96 h-[calc(100vh-1.5rem)] max-h-[600px] bg-[#1a1a1a]/90 flex flex-col overflow-hidden">
          ...
          <div className="relative bg-gradient-to-r from-[#FF9000] to-[#FF5E00] p-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-bold text-lg">영남대 챗봇</h3>
            </div>
            <button

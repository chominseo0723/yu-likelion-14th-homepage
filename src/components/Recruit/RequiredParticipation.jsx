...
import { mediumStyle, boldStyle, titleStyle } from "../../styles/typography";
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[1040px] mx-auto">
        ...
        <div className="mt-[110.16px] max-md:mt-10 grid grid-cols-2 max-md:grid-cols-1 gap-x-[19px] gap-y-[31px] max-md:gap-y-4">
          {RequiredParticipationData.map((text, index) => (
            <div
              key={index}
              className={`w-full max-w-[511px] max-md:max-w-full h-[106px] max-md:h-auto max-md:min-h-[80px] rounded-[30px] pl-[31px] max-md:pl-5 pr-[31px] max-md:pr-5 flex items-center justify-start box-border glass glass-ideal ${index === 0 ? "" : index === 1 ? "" : "max-md:mt-0"}`}
            >
              <p
